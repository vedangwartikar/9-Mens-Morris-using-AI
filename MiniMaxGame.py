...
def MaxMin(board, depth):
    ...
    if depth:
        v = -math.inf
        depth -= 1
        for possible_move in generate_moves_midgame_endgame(board):
            minmax = MinMax(possible_move, depth)
            static_estimate = static_estimation_midgame_endgame(minmax)
...
def MinMax(board, depth):
    ...
    if depth:
        ...
        for possible_move in generate_moves_midgame_endgame(board):
            ...
            static_estimate = static_estimation_midgame_endgame(maxmin)
            global positions_evaulated
            positions_evaulated += 1
            if v > static_estimate:
                v = static_estimate
