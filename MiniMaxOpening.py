...
def MaxMin(board, depth):
    ...
    if depth:
        v = -math.inf
        depth -= 1
        for possible_move in board_obj.generate_moves_opening(board):
            minmax = MinMax(possible_move, depth)
            static_estimate = static_estimation_obj.static_estimation_opening(minmax)
...
def MinMax(board, depth):
    ...
    if depth:
        ...
        for possible_move in board_obj.generate_moves_opening(board):
            ...
            static_estimate = static_estimation_obj.static_estimation_opening(maxmin)
            global positions_evaulated
            positions_evaulated += 1
            if v > static_estimate:
                v = static_estimate
